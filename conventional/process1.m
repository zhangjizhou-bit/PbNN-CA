...
for idx = 1:length(solvers)
    solver = solvers{idx};
    for noise_level = 0:1e-5:2e-4
    % for noise_level = 1e-5:1e-5:1e-5
        fprintf('noise_level: %1.1e\n',noise_level);    
        load(['../experiment1_results/imaged_intens_',num2str(noise_level,'%1.1e'),'.mat']);
        if solver == "GS_cos4"
        ...
        save(['../experiment1_results/',varables.solver,'_',num2str(noise_level,'%1.1e'),'.mat'],...
